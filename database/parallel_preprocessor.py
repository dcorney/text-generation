...
# Just for comparison tests
def serial(file_ids):
    for id in file_ids:
        ts = process_one_file(id)
        print("{} has {} tokens".format(id, len(ts)))


def process_parallel(file_ids):
    with concurrent.futures.ProcessPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(process_one_file, file_id): file_id for file_id in file_ids}
...
def dev():
    ...
    print("\nSerial {:0.3f}   Parallel {:0.3f}   Ratio {:0.3f}".format(
        s_elapsed_time, p_elapsed_time, p_elapsed_time / s_elapsed_time))
